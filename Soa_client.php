...
class Soa_Client {
	...
	protected function request($send, $retObj) {
		...
		//发送失败了
		if ($retObj->socket->send(self::encode($retObj->send, self::DECODE_PHP, 0, $retObj->requestId)) === false) {
			...
			if ($n > 0) {
				...
				foreach ($read as $sock) {
					...
					if (!isset($buffer[$id])) {
						...
						$header[$id] = unpack(self::HEADER_STRUCT, substr($data, 0, self::HEADER_SIZE));
						//错误的包头
						...
						$buffer[$id] = substr($data, self::HEADER_SIZE);
					} else {
						$buffer[$id] .= $data;
					}
					//达到规定的长度
					if (strlen($buffer[$id]) == $header[$id]['length']) {
						...
						//成功处理
						$this->finish(self::decode($buffer[$id], $header[$id]['type']), $retObj);
						$success_num++;
					}
					//继续等待数据
				}
